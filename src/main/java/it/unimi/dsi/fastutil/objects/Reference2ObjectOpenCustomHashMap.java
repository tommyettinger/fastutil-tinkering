...
public class Reference2ObjectOpenCustomHashMap <K,V> extends AbstractReference2ObjectMap <K,V> implements java.io.Serializable, Cloneable, Hash {
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2ObjectMap.Entry <K,V> > implements FastEntrySet <K,V> {
	 ...
	 public boolean contains(final Object o) {
	  ...
	  if (( strategy.equals( (k), (null) ) )) return Reference2ObjectOpenCustomHashMap.this.containsNullKey && ( (value[n]) == null ? (v) == null : (value[n]).equals(v) );
	  K curr;
	  final K[] key = Reference2ObjectOpenCustomHashMap.this.key;
	  int pos;
	  // The starting point.
	  if (( (curr = key[pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask]) == (null) )) return false;
	  if (( strategy.equals( (k), (curr) ) )) return ( (value[pos]) == null ? (v) == null : (value[pos]).equals(v) );
	  // There's always an unused entry.
	  while(true) {
	   if (( (curr = key[pos = (pos + 1) & mask]) == (null) )) return false;
	   if (( strategy.equals( (k), (curr) ) )) return ( (value[pos]) == null ? (v) == null : (value[pos]).equals(v) );
